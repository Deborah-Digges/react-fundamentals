...
- Using a `key` for items in lists is important because it helps react partially
  reconstruct the DOM if something is added or removed from the list
- Use a unique key for items when possible
- Using zero indices of list items for keys is React's default behaviour and
  will not help if you are having issues related to this
