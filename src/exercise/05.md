...
- `className` is used to apply CSS classes to an element. The CSS file must be
  imported. i.e. the class must be "visible" where it is used
- The equivalent of inline styles for a JSX component is achieved using the
  `style` property which accepts a style object. E.x.:
  `style={{ backgroundColor: 'red' }}`.
- The CSS properties used in the `style` prop are `camelCased` instead of
  `kebab-cased`.
