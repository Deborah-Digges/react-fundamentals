...
- A component is the element of reuse in React. It encapsulates JSX and related
  logic - if you wanted to repeat the same JSX in multiple places
- A custom component name can be passed to
  `React.createElement(ComponentName, domElement)`. However, it is important
  that the component name is in uppercase, otherwise it will render it as an
  HTML tag: `<name></name>`.
- We can add validation to props using the `PropTypes` package. It has several
  built in prop types, but you can also write a custom validator with the sytax:
  `(props, propName, componentName) => {}` which returns an `Error` if
  validation fails.
- A `React.Fragment` can be used to render sibling elements. The shorthand is
  `<></>`. React fragments are not rendered in the actual DOM
