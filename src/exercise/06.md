...
- A form has a `handleSubmit` prop which takes an `event` as the default
  argument. Call `event.preventDefault()` to prevent the default form
  behaviour(reloading).
- To get an element from a particular form element:
  `event.target.elements[0].value` or
  `event.target.elements.elementNameOrId.value`
- Another way of getting the value is using a ref( this is not the best way to
  do it as refs are an escape hatch for when we need to access dom nodes to
  perform some imperative operation on them like animations, blurring,
  focussing). A ref is an object which persists across the lifecycle of a
  component. It's `current` value (`ref.current.value`) refers the dom element
  which was rendered for an element at that time (`<div ref={ref}>`). `ref` will
  point to the underlying dom node rendered for this `div`
- Use the `onChange` prop on an individual form element like an `input` to
  handle validation: `<input onChange={handleChange}>`. The callback takes on
  default `event` value. To get the value from the event, use
  `event.target.value`.
- An "uncontrolled" component is one whose state change is managed completely by
  the broswer. We can be notified of changes to an uncontrolled component by
  subscribing to them, but it is possible for the two (our version of what's
  stored vs what's actually stored) to get out of sync. To force update an
  uncontrolled component, we need to do something imperative like
  `node.value = "something"`
- A "controlled" component is one whose value is tied to a variable and there is
  no chance of the two going out of sync. It can declaratively be defined as
  `<input value={var}>`. This `var` is typically a state variable that's
  initialized to a sensible default. The value is updated by listening on the
  same events(ex. `onChange`) and setting the variable's value from it.
- UX learning: controlled components can be used to sanitize user input
  programmatically without having to show an error message to the user. Ex: if
  uppercase letters are not allowed, we could programatically lowercase an input
  and set the state var to that value.
